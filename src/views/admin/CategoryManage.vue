  ...
  <div class="p-5">
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
      <div class="p-4 border-b border-gray-200 flex justify-between items-center">
        ...
        <el-button type="success" @click="openCreateDialog">新增分类</el-button>
      </div>
...
const loadCategories = async () => {
  loading.value = true
  try {
    const result = await adminCategoryApi.getCategories(currentPage.value, pageSize.value, searchName.value)
...
const handleSubmit = async () => {
  ...
  try {
    let result
    if (isEditing.value) {
      result = await adminCategoryApi.updateCategory(editingId.value, formData.name, formData.status)
    } else {
      result = await adminCategoryApi.createCategory(formData.name)
    }
...
const handleDelete = (row) => {
  ElMessageBox.confirm(
    ...
    try {
      const result = await adminCategoryApi.deleteCategory(row.id)
      if (result.success) {
